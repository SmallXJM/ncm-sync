...
class DownloadProcess:
    ...
    async def _prepare_job_tasks(self, job: DownloadJob, fetch_result: dict) -> Tuple[
        ...
        effective_ids = fetch_result.get("effective_ids", [])  # 过滤后的有效歌曲 ID（排除作业内已有任务）
        ...
        return tasks_data, detail_map, failed_ids, fetch_result.get("skipped_existing_ids", []), copied_task_ids

    async def _handle_batches(self, job: DownloadJob, tasks_data: List[dict], detail_map: Dict[str, dict],
                              batch_size: int) -> int:
    ...
    async def _create_pending_batch(self, job: DownloadJob, batch: List[dict]) -> List[DownloadTask]:
        """批量创建待下载任务并更新作业统计。"""
        created: List[DownloadTask] = []  # 创建后的任务列表
        async with self.uow_factory() as uow:
            created = await self.task_repo.create_batch(uow.session, batch)
            await self.job_repo.update_statistics(uow.session, job.id,
                                                  total_tasks=(job.total_tasks or 0) + len(created))
        return created
    ...
    async def _try_copy_existing(self, job: DownloadJob, data: dict) -> int | None:
        ...
        async with self.uow_factory() as uow:
            ...
            if not source_task or not source_task.file_path:
                return None
        lock = self._copy_locks.setdefault(data["music_id"], asyncio.Lock())  # 针对 music_id 的复制锁
        async with lock:
            async with self.uow_factory() as uow:
                try:
                    ...
                    try:
                        if 'target_path' in locals() and os.path.exists(str(target_path)):
                            os.remove(str(target_path))  # 清理失败复制的残留目标文件
                    except Exception:
                        pass
                    logger.warning(f"Copy optimization failed for music {data['music_id']}: {ce}")
                    return None

    async def _dispatch_and_wait_batch(self, job: DownloadJob, created: List[DownloadTask]) -> None:
        """调度当前批次的下载工作流并等待所有子流程完成；收集异常并记录日志。"""
        # 调度执行工作流
        batch_futures: List[asyncio.Task] = []  # 子任务 Future 列表
        for task in created:
            future = asyncio.create_task(self.orch._execute_download_workflow(task.id, job.target_quality))
    ...
    async def _fetch_playlist_tracks(self, job_id: int, playlist_id: str, max_retries: int = 3) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if status in (429, 503):
                    ...
                    continue
                    # 触发降级路径；返回空结果以避免阻塞
                return {"tracks": [], "effective_ids": [], "failed_ids": [], "skipped_existing_ids": []}
            except Exception as e:
                logger.warning(f"Fetch playlist detail failed for {playlist_id} (attempt {attempt + 1}): {e}")
                await asyncio.sleep(delay)
                delay = min(delay * 2, 8.0)
        ...
        effective_ids = [mid for mid in all_ids if mid not in existing_ids]  # 作业内新任务的歌曲 ID
        skipped_existing_ids = [mid for mid in all_ids if mid in existing_ids]  # 已存在任务的歌曲 ID
        ...
        for i in range(0, len(effective_ids), chunk_size):
            ...
            try:
                ...
                logger.warning(f"Fetch song details failed for chunk starting {chunk[0]}: {e}")
                failed_ids.extend(chunk)
        return {
