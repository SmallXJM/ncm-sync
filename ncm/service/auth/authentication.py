...
async def batch_upload_cookies(cookies: List[str], login_status_service, **kwargs) -> Dict[str, Any]:
    """
    Batch upload and validate multiple cookies.

    Args:
        cookies: List of cookie strings to upload and validate
        login_status_service: Service to validate cookies
    ...
    try:
        ...
        for i, cookie in enumerate(cookies):
            try:
                ...
                if user_info:
                    # Save user login
                    result = await login_status_service.save_user_login(user_info, cookie, "batch_upload")
                ...
                results.append({
                    ...
                    "success": False,
                    "error": str(e),
                    "message": f"处理 Cookie {i + 1} 失败"
                })

        return {
            "success": True,
            "code": 200,
            "message": f"批量上传完成，成功 {success_count}/{len(cookies)} 个",
            "data": {
                "total_cookies": len(cookies),
                "success_count": success_count,
                "failed_count": len(cookies) - success_count,
                "results": results
            }
        }
