    ...
    fields: [
      {
        ...
        label: '定时任务调度',
        description: '启用后将按照 Cron 表达式自动执行下载任务。关闭后仅支持手动触发。',
      ...
      },
      {
        ...
        label: 'Cron 表达式',
        description:
          '标准的 5 段或 6 段 Cron 表达式（分 时 日 月 周 [年]）。例如：0 2 * * * 表示每天凌晨 2 点执行。',
      ...
      },
      {
        ...
        label: '最大并发下载数',
        description: '同时进行的下载任务数量（1 ~ 100）。数值过大可能导致网络拥堵或被封禁。',
        control: { type: 'intRange', min: 1, max: 100 },
        rule: { kind: 'intRange', min: 1, max: 100, label: '最大并发量' },
      },
      {
        id: 'download.max_threads_per_download',
        path: 'download.max_threads_per_download',
        label: '单任务线程数',
        description: '单个下载任务使用的最大线程数（1 ~ 64）。建议设置为 2-10 之间。',
        control: { type: 'intRange', min: 1, max: 64 },
        rule: { kind: 'intRange', min: 1, max: 64, label: '单任务最大线程数' },
      },
      {
        id: 'download.temp_downloads_dir',
        path: 'download.temp_downloads_dir',
        label: '临时下载目录',
        description: '下载过程中的临时文件存放位置。支持绝对路径或相对于运行目录的相对路径。',
    ...
    fields: [
      {
        ...
        label: '文件名模板',
        description: '歌曲文件的命名格式。支持变量：{artist}, {title}, {album} 等。',
      ...
      },
      {
        ...
        label: '歌单目录前缀',
        description: '歌单下载时的目录层级结构。支持变量：{user_name}, {playlist_name}。',
